...
# def parse_conversion(raw):
#     """Parse conversion input from frontend (percentage) to decimal"""
#     if raw is None or str(raw).strip() == "":
#         return None
#     s = str(raw).strip()
#     try:
...
#             return float(s.replace("%", "").strip()) / 100.0
#         v = float(s)
#         # divide by 100 if user inputs 1-100
#         return v / 100.0 if v > 1 else v
#     except:
#         return None
...
#     # prepare stages for percentage display in input fields
#     stages_display = []
#     for s in stages:
#         stages_display.append({
#             "name": s["name"],
#             "conversion": f"{s['conversion']*100:.1f}" if s["conversion"] is not None else ""
#         })
...
def forward_calc(stages, starting_volume, average_deal_size):
    ...
    for i, stage in enumerate(stages):
        ...
        rows.append({
            ...
            "Stage Volume": int(round(vols_float[i])),
            "Conversion to Next": conv_display,
            "Values": f"{stage['name']}: {int(round(vols_float[i]))}"
        })
...
def reverse_calc(stages, target_revenue, average_deal_size):
    ...
    for i, stage in enumerate(stages):
        ...
        if i == 0:
            ...
            prev = required_float[i - 1] if required_float[i - 1] else 1
            cum_pct = (required_float[i] / prev * 100.0) if prev else 0.0
        rows.append({
            "Stage Name": stage["name"],
            "Required Volume": req_disp,
            "Cumulative Required Conversion": f"{cum_pct:.1f}%"
