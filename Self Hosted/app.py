...
def register_site_form():
    return render_template_string(
        ...
        <!DOCTYPE html>
        <html>
        <head><title>Register WordPress Site</title></head>
        <body>
...
def schedule_post_form():
    ...
    <body>
        ...
        <form method="post" action="/schedule-post" enctype="multipart/form-data">
            ...
            <label>Content:<br>
                <textarea name="content_html" rows="6" cols="60"></textarea>
            </label><br><br>
            <label>Upload Image: <input type="file" name="image_file"></label><br><br>
...
def schedule_post_form_handler():
    data = request.form.to_dict()
    file = request.files.get("image_file")
    if file and file.filename:
        filename = secure_filename(file.filename)
        image_bytes = file.read()
        data["image_file"] = (filename, image_bytes)
...
def handle_schedule(data, from_form=False):
    ...
    embed = bool(data.get("embed_image_in_content"))
    ...
    try:
        if from_form and "image_file" in data:
            ...
            media = upload_media(site, image_bytes, filename)
            featured_media_id = media.get("id")
            media_src_url = media.get("source_url")
        elif data.get("image_url"):
            r = requests.get(data["image_url"], timeout=60)
            r.raise_for_status()
            filename = os.path.basename(urlparse(data["image_url"]).path) or "image"
